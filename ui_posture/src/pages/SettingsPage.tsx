...
import { Sun, Moon, Monitor, Camera, RefreshCw, Bell, Save, MapPin, Sliders, HardDrive, AlertTriangle, Users, Sparkles } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  return (
    <div className="p-lg space-y-lg pb-32 max-w-3xl">
          ...
          <>
            <div className="border-t border-outline-variant/30 pt-md">
              <div className="flex items-center gap-sm">
                <h2 className="text-headline-md font-bold text-on-surface">Deployment Configuration</h2>
                <span className="inline-flex items-center gap-1 rounded-full bg-amber-100/20 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-amber-500 border border-amber-500/20">
            ...
            <SettingSection icon={MapPin} title="Camera Mapping">
              <select disabled value={settings.cameraMapping} className="bg-surface-container-high border border-outline-variant rounded-lg px-md py-sm text-body-sm text-on-surface-variant opacity-60 cursor-not-allowed outline-none">
                <option value="zone">By Zone (A, B, C, D)</option>
                <option value="workstation">By Workstation</option>
                <option value="worker">By Worker Assignment</option>
              </select>
            </SettingSection>

            <SettingSection icon={Users} title="Workstation Mapping">
              <select disabled value={settings.workstationMapping} className="bg-surface-container-high border border-outline-variant rounded-lg px-md py-sm text-body-sm text-on-surface-variant opacity-60 cursor-not-allowed outline-none">
            ...
            <SettingSection icon={AlertTriangle} title="Alert Threshold">
              <div className="flex gap-sm opacity-60 pointer-events-none">
                {(['low', 'moderate', 'high'] as const).map((t) => (
                  <button key={t} className={`px-md py-sm rounded-lg border text-body-sm font-medium transition-all ${settings.alertThreshold === t ? 'border-primary/50 bg-primary/10 text-primary' : 'border-outline-variant text-on-surface-variant hover:text-on-surface'}`}>
            ...
            <SettingSection icon={HardDrive} title="Data Retention">
              <select disabled value={settings.dataRetention} className="bg-surface-container-high border border-outline-variant rounded-lg px-md py-sm text-body-sm text-on-surface-variant opacity-60 cursor-not-allowed outline-none">
      ...
      <button onClick={handleSave} disabled={!dirty} className={`flex items-center gap-sm px-lg py-md rounded-lg font-body-md font-bold transition-all ${dirty ? 'bg-primary text-on-primary hover:brightness-110' : 'bg-surface-container-high text-on-surface-variant cursor-not-allowed'}`}>
